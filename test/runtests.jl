...
@testset "Error" begin
    for i in 1:21
        t = convert(EzXMLp.NodeType, i)
        @test t == i
        @test occursin(r"^[A-Z_]+_(NODE|DECL|START|END)$", repr(t))
        @test string(t) == string(i)
        @test convert(EzXMLp.NodeType, t) === t
    end
    @test_throws AssertionError repr(convert(EzXMLp.NodeType, 0))
    @test_throws AssertionError repr(convert(EzXMLp.NodeType, 100))

    err = EzXMLp.XMLError(1, 77, "some parser error", EzXMLp.XML_ERR_ERROR, 123)
    @test isa(err, EzXMLp.XMLError)
    buf = IOBuffer()
    showerror(buf, err)
...
@testset "Reader" begin
    @testset "XML" begin
        ...
        @test isa(doc, EzXMLp.Document)
        @test nodetype(doc.node) === EzXMLp.DOCUMENT_NODE
        @test nodetype(readxml(valid_file).node) === EzXMLp.DOCUMENT_NODE
        @test_throws EzXMLp.XMLError readxml(invalid_file)
        @assert !isfile("not-exist.xml")
        @test_throws EzXMLp.XMLError readxml("not-exist.xml")

        # from compressed file
        compressed = joinpath(dirname(@__FILE__), "sample1.xml.gz")
        @test isa(readxml(compressed), EzXMLp.Document)

        # from stream
        doc = open(readxml, valid_file)
        @test isa(doc, EzXMLp.Document)
        @test nodetype(doc.node) === EzXMLp.DOCUMENT_NODE
        @test_throws EzXMLp.XMLError open(readxml, invalid_file)
    ...
    @testset "HTML" begin
        ...
        @test isa(doc, EzXMLp.Document)
        @test nodetype(doc.node) === EzXMLp.HTML_DOCUMENT_NODE
        @test nodetype(readhtml(valid_file).node) === EzXMLp.HTML_DOCUMENT_NODE
        @assert !isfile("not-exist.html")
        @test_throws EzXMLp.XMLError readxml("not-exist.html")
        @test_throws EzXMLp.XMLError readhtml("not-exist.html")

        # from compressed file
        compressed = joinpath(dirname(@__FILE__), "sample1.html.gz")
        @test isa(readxml(compressed), EzXMLp.Document)
        @test isa(readhtml(compressed), EzXMLp.Document)
...
@testset "Parser" begin
    @testset "XML" begin
        ...
        @test nodetype(parsexml("<xml/>").node) === EzXMLp.DOCUMENT_NODE
        @test nodetype(parsexml("<html/>").node) === EzXMLp.DOCUMENT_NODE
        @test nodetype(parsexml(b"<xml/>").node) === EzXMLp.DOCUMENT_NODE
        @test nodetype(parsexml(b"<html/>").node) === EzXMLp.DOCUMENT_NODE
        ...
        @test_throws EzXMLp.XMLError parsexml(" ")
        @test_throws EzXMLp.XMLError parsexml("abracadabra")
        @test_throws EzXMLp.XMLError parsexml("""<?xml version="1.0"?>""")

        _, messages = capture_logging_messages() do
            @test_throws EzXMLp.XMLError parsexml("<gepa?>jgo<<<><<")
        end
    ...
    @testset "HTML" begin
        ...
        @test nodetype(parsehtml("<html/>").node) === EzXMLp.HTML_DOCUMENT_NODE
        @test nodetype(parsehtml(b"<html/>").node) === EzXMLp.HTML_DOCUMENT_NODE
...
@testset "Stream Reader" begin
    for i in 0:17
        t = convert(EzXMLp.ReaderType, i)
        @test t == i
        @test occursin(r"READER_[A-Z_]+$", repr(t))
        @test string(t) == string(i)
        @test convert(EzXMLp.ReaderType, t) === t
    end
    @test_throws AssertionError repr(convert(EzXMLp.ReaderType, -1))
    @test_throws AssertionError repr(convert(EzXMLp.ReaderType, 18))

    sample2 = joinpath(dirname(@__FILE__), "sample2.xml")
    reader = open(EzXMLp.StreamReader, sample2)
    @test isa(reader, EzXMLp.StreamReader)
    typs = []
    names = []
    depths = []
    contents = []
    ...
    for typ in reader
        ...
        if typ == EzXMLp.READER_ELEMENT && nodename(reader) == "elm"
            push!(contents, nodecontent(reader))
            push!(attributes, reader["attr1"])
        end
        @test isa(expandtree(reader), EzXMLp.Node)
    end
    @test typs[1] === EzXMLp.READER_ELEMENT
    @test typs[2] === EzXMLp.READER_SIGNIFICANT_WHITESPACE
    @test names[1] == "root"
    @test names[3] == "elm"
    @test depths[1] === 0
    @test depths[end] === 0
    ...
    @test open(collect, EzXMLp.StreamReader, sample2) == typs

    simple_graphml = joinpath(dirname(@__FILE__), "simple.graphml")
    reader = open(EzXMLp.StreamReader, simple_graphml)
    @test isa(reader, EzXMLp.StreamReader)
    typs = []
    names = []
    namespaces = []
    for typ in reader
        push!(typs, typ)
        push!(names, nodename(reader))
        if typ == EzXMLp.READER_ELEMENT
            push!(namespaces, namespace(reader))
        end
        @test isa(expandtree(reader), EzXMLp.Node)
    end
    @test first(typs) === EzXMLp.READER_COMMENT
    @test first(names) == "#comment"
    @test last(typs) === EzXMLp.READER_END_ELEMENT
    ...
    reader = EzXMLp.StreamReader(input)
    typs = []
    names = []
    for typ in reader
        push!(typs, typ)
    ...
    for typ in reader
        ...
        if reader.name == "head" && reader.:type == EzXMLp.READER_ELEMENT
            @test reader.depth == 1
            @test reader.namespace == "http://www.w3.org/1999/xhtml"
        elseif reader.name == "title" && reader.:type == EzXMLp.READER_ELEMENT
            @test reader.depth == 2
            @test reader.content == "Hey"
            @test reader.namespace == "http://www.w3.org/1999/xhtml"
        elseif reader.:type == EzXMLp.READER_END_ELEMENT
    ...
    @test_throws EzXMLp.XMLError iterate(EzXMLp.StreamReader(IOBuffer("not xml")))

    # memory management test
    for _ in 1:10
        reader = EzXMLp.StreamReader(IOBuffer("<a><b/></a>"))
        for typ in reader
            if typ == EzXMLp.READER_ELEMENT && EzXMLp.nodename(reader) == "a"
                a = EzXMLp.expandtree(reader)
                b = EzXMLp.firstelement(a)
            end
        end
        close(reader)
    end
    ...
    reader = EzXMLp.StreamReader(IOBuffer("""
    ...
    for typ in reader
        ...
        elseif typ == EzXMLp.READER_END_ELEMENT
    ...
    #@assert !isfile("not-exist.xml")
    #@test_throws EzXMLp.XMLError open(EzXMLp.StreamReader, "not-exist.xml")
end

@testset "Constructors" begin
    n = XMLDocumentNode("1.0")
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.DOCUMENT_NODE
    @test document(n) === EzXMLp.Document(n.ptr)

    n = HTMLDocumentNode(nothing, nothing)
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.HTML_DOCUMENT_NODE
    @test document(n) === EzXMLp.Document(n.ptr)
    ...
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.ELEMENT_NODE
    @test iselement(n)
    ...
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.TEXT_NODE
    @test istext(n)
    @test_throws ArgumentError document(n)
    ...
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.COMMENT_NODE
    @test iscomment(n)
    @test_throws ArgumentError document(n)
    ...
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) === EzXMLp.CDATA_SECTION_NODE
    @test iscdata(n)
    @test_throws ArgumentError document(n)
    ...
    @test isa(n, EzXMLp.Node)
    @test n.owner == n
    @test nodetype(n) == EzXMLp.ATTRIBUTE_NODE
    @test isattribute(n)
    @test_throws ArgumentError document(n)
    ...
    @test isdtd(n)
    @test n.owner === n
    @test nodetype(n) === EzXMLp.DTD_NODE
...
@testset "Traversal" begin
    ...
    @test hasroot(doc)
    @test !hasdtd(doc)
    @test isa(root(doc), EzXMLp.Node)
    @test root(doc) == root(doc)
    @test root(doc) === root(doc)
    @test hash(root(doc)) === hash(root(doc))
    @test nodetype(root(doc)) === EzXMLp.ELEMENT_NODE
    ...
    """)
    r = root(doc)
    @test nodetype(firstnode(r)) === EzXMLp.TEXT_NODE
    @test nodetype(lastnode(r)) === EzXMLp.TEXT_NODE
    @test nodetype(firstelement(r)) === EzXMLp.ELEMENT_NODE
    @test nodename(firstelement(r)) == "c1"
    @test nodetype(lastelement(r)) === EzXMLp.ELEMENT_NODE
    @test nodename(lastelement(r)) == "c3"
    c1 = firstelement(r)
    @test hasnextnode(c1)
    @test hasprevnode(c1)
    @test nodetype(nextnode(c1)) === EzXMLp.TEXT_NODE
    @test nodetype(prevnode(c1)) === EzXMLp.TEXT_NODE
...
@testset "Properties" begin
    ...
    doc = parsexml("<root/>")
    @test doc.node isa EzXMLp.Node
    @test doc.node.type === EzXMLp.DOCUMENT_NODE
    @test doc.node.name === nothing
    @test doc.node.path == "/"
    @test doc.node.content == ""
    @test doc.node.namespace === nothing
    @test doc.root isa EzXMLp.Node
    @test doc.root.type === EzXMLp.ELEMENT_NODE
    ...
    @test attr.type === EzXMLp.ATTRIBUTE_NODE
    @test attr.name == "attr"
    ...
    @test doc.root.firstnode.type === EzXMLp.TEXT_NODE
    @test doc.root.lastnode.type === EzXMLp.TEXT_NODE
...
@testset "XPath" begin
    ...
    @test_throws EzXMLp.XMLError findall("//bar!", doc)
...
@testset "Misc" begin
    @testset "show" begin
        ...
        @test occursin(r"^EzXMLp.Node\(<DOCUMENT_NODE>\)$", repr(doc.node))
        @test occursin(r"^EzXMLp.Node\(<ELEMENT_NODE\[root\]>\)$", repr(root(doc)))
        @test occursin(r"^EzXMLp.Node\(<ATTRIBUTE_NODE\[attr\]>\)$", repr(attributes(root(doc))[1]))
        @test occursin(r"^EzXMLp.Document\(EzXMLp.Node\(<DOCUMENT_NODE>\)\)$", repr(doc))

        sample2 = joinpath(dirname(@__FILE__), "sample2.xml")
        reader = open(EzXMLp.StreamReader, sample2)
        @test occursin(r"^EzXMLp.StreamReader\(<[A-Z_]+@0x[a-f0-9]+>\)$", repr(reader))
        close(reader)
    end
...
@test isempty(EzXMLp.XML_GLOBAL_ERROR_STACK)
